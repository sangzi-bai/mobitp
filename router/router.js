...
var pageamount = require("../settings.js").pageamount;
//首页
exports.showIndex = function (req, res, next) {
    ...
    db.find("fbtupian", {},{"pageamount":pageamount,"page":page,"sort":{"datetime":1}},function (err, result) {
        if (result.length == 0) {
        ...
        return;
            ...
            db.getAllCount("fbtupian",function(count){
                res.render("indexs", {
                "login": login,
                "username": username,
                "active": "首页",
                "result": result,    //数组
                "pageamount" : Math.ceil(count / pageamount),
                "page":page   
                });
...
exports.dosetavatar = function (req, res, next) {
    ...
    form.parse(req, function (err, fields, files) {
        ...
        var tupianjj=fields.tupianjj;
        var extname = path.extname(files.tupian.name);
        var ttt = sd.format(new Date(), 'YYYYMMDDHHmmss');
            var ran = parseInt(Math.random() * 89999 + 10000);
        var bl=ttt+ran+extname;
        //console.log(oldpath+extname);
        var newpath = path.normalize(__dirname + "/../avatar") + "/"  +bl;
        fs.rename(oldpath, newpath, function (err) {
            if (err) {
                res.send("失败");
                return;
            }
             db.insertOne("fbtupian", {
                    "username": username,
                    "tupianjj":tupianjj,
                    "datetime": new Date(),
                    "tupian": "/avatar/"+ ttt + ran + extname
                }, function (err, result) {
                    if (err) {
                        res.send("图片上传失败");
                        return;
